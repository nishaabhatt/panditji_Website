...
const Services = ({ data }) => {
  // Create an array to store unique poojas
  const uniquePoojas = [];

  // Iterate through the data and filter out duplicates based on the name
  data.servicePages?.forEach((item) => {
    const existingPooja = uniquePoojas.find((pooja) => pooja.name === item.name);

    // Add pooja to the array if it's not already present
    if (!existingPooja) {
      uniquePoojas.push(item);
    }
  });
